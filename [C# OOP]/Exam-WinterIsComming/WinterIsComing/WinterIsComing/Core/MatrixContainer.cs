namespace WinterIsComing.Core
{
    ...
    public class MatrixContainer : IUnitContainer
    {
        ...
        public IEnumerable<IUnit> GetUnitsInRange(int x, int y, int range)
        {
            ...
            var startRow = Math.Max(0, y - range);
            var endRow = Math.Min(this.unitMatrix.GetLength(0) - 1, y + range);
            var startCol = Math.Max(0, x - range);
            var endCol = Math.Min(this.unitMatrix.GetLength(1) - 1, x + range);
        ...
        public void Add(IUnit unit)
        {
            ...
            {
                throw new GameException(string.Format("There is already a unit on position [{0},{1}]", unit.X, unit.Y));
            }
        ...
        public void Move(IUnit unit, int newX, int newY)
        {
            ...
            {
                throw new GameException(string.Format("There is already a unit on position [{0},{1}]", newX, newY));
            }
        ...
        {
            if (!(x >= 0 && x < this.unitMatrix.GetLength(1)) || !(y >= 0 && y < this.unitMatrix.GetLength(0)))
            {
