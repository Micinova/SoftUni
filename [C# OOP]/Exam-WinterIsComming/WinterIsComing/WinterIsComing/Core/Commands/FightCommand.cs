    ...
    public class FightCommand : AbstractCommand
    {
        ...
        public override void Execute(string[] commandArgs)
        {
            ...
            foreach (var currentUnit in this.Engine.Units)
            {
                ...
                var unitsInRange =
                    this.Engine.UnitContainer.GetUnitsInRange(currentUnit.X, currentUnit.Y, currentUnit.Range)
        ...
        private void ProcessBattle(IUnit currentUnit, IEnumerable<IUnit> targets, IList<IUnit> killedUnits)
        {
            foreach (var target in targets)
            {
                ...
                var healthDamage = attack.Damage - target.DefensePoints;
                if (healthDamage > 0)
                {
                    target.HealthPoints -= healthDamage;

                    this.Engine.OutputWriter.Write(
                        string.Format(
                    ...
                    if (target.HealthPoints < 0)
                    {
                        ...
                        this.Engine.OutputWriter.Write(
                            string.Format("{0} has killed {1}", currentUnit.Name, target.Name));
